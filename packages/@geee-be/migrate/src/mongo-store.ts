...
export class MongoStorage implements Storage {
  private readonly migrations: Collection<{ _id: string; digest: string; when: Date }>;

  constructor(db: Db) {
    this.migrations = db.collection('_migrations');
  }

  public async check(fileName: string, digest: string): Promise<{ applied: boolean; current: boolean }> {
  ...
  public async applied(fileName: string, digest: string): Promise<void> {
    await this.migrations.updateOne({ _id: fileName }, { $set: { digest, when: new Date() } }, { upsert: true });
  }
