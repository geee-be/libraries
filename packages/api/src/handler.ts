...
export class Handler<T extends Entity> {
  constructor(
    ...
    protected readonly options: MutationOptions<T> = {}
  ) { }
  ...
  public patchOne(): PatchOneHandler<Partial<T>> {
    return async (filter, patch) => {
      ...
      const result = await this.collection.findOneAndUpdate(filter as Filter<T>, { $set: this.mutatePatch(patch) as any }, { returnDocument: 'after' });
  ...
  protected mutatePatch(patch: Partial<T>): Partial<T> {
    return this.options.mutatePatch
      ? this.options.mutatePatch(patch)
      : patch;
  }

  protected mutateResult(result: Entity): unknown {
    return this.options.mutateResult
      ? this.options.mutateResult(result)
      : result;
  }
}
