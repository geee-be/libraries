...
export const OMIT_PRETTY = ['file', 'hostname', 'line', 'message', 'pid', 'timestamp', 'host', 'ip', 'spanId', 'traceId'];
...
export class ConsoleWriter implements LogWriter {
  ...
  public write(log: Log): void {
    ...
    if (this.options.pretty) {
      process.nextTick(() => console.log((log?.timestamp ?? new Date()).toISOString(), log?.message ?? '', omit(log, this.options.prettyOmit || OMIT_PRETTY)));
      return;
    }
